...
ALTER TABLE
  `course_offering`
ADD
  CONSTRAINT `fk_coord_id` FOREIGN KEY (`coordinator_id`) REFERENCES `user` (`u_id`);
...
ALTER TABLE
  `timetable`
ADD
  CONSTRAINT `fk_c_code` FOREIGN KEY (`c_code`) REFERENCES `course_offering` (`c_code`);

ALTER TABLE
  `timetable`
ADD
  CONSTRAINT `fk_f_id` FOREIGN KEY (`f_id`) REFERENCES `facility` (`f_id`);
...
ALTER TABLE
  `upcoming_event`
ADD
  CONSTRAINT `fk_admin_id` FOREIGN KEY (`added_by`) REFERENCES `user`(`u_id`);
...
INSERT INTO
  `timetable` (`c_code`, `f_id`, `lec_day`, `lec_time`)
VALUES
