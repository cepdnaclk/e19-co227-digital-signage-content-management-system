<?php include_once $_SERVER['DOCUMENT_ROOT'] . "/config.php" ?>


<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="/css/styles.css">
    <link rel="stylesheet" href="/css/login.css">
    <?php include APP_ROOT . "/includes/header_core.php" ?>
    <title>IT Center | Register</title>
    <style>
        .alert {
            left: 50%;
        }
    </style>
</head>

<body>
    <div class="login">
        <?php if (isset($_GET['error'])) { ?>
            <div class="alert error">
                <h4>Error :</h4>
                <p><?= $_GET['error'] ?></p>
            </div>
        <?php } ?>
        <?php if (isset($_GET['success'])) { ?>
            <div class="alert success">
                <h4>Success :</h4>
                <p><?= $_GET['success'] ?></p>
            </div>
        <?php } ?>
        <div class="left">
            <h3>Admin Portal</h3>
            <h5>Digital Signage CMS</h5>
            <p>Welcome to the Admin Portal of the DSCMS. Here you can manage and update all the content related to your
                board contents</p>
            <span>Need help? <a href="./support.php">Contact us</a></span>
        </div>
        <div class="content">

            <form action="/backend/register.php" method="POST">
                <h1>Register</h1>
                <div class="login-input">
                    <label for="user_name">Username</label>
                    <input type="text" id="user_name" name="user_name" placeholder="John Doe" required>
                </div>
                <div class="login-input">
                    <label for="password">Password</label>
                    <input type="password" id="password" name="password" placeholder="xxxxxxxx" required>
                </div>
                <div class="login-input">
                    <label for="rpassword">Confirm Password</label>
                    <input type="password" id="rpassword" name="rpassword" placeholder="xxxxxxxx" required>
                </div>
                <div class="login-input">
                    <label for="email">Email</label>
                    <input type="email" id="email" name="email" placeholder="email" required>
                </div>
                <div class="login-input">
                    <label for="contact">Contact</label>
                    <input type="contact" id="contact" name="contact" placeholder="contact" required>
                </div>
                <button type="submit" name="register">Register</button>
            </form>
        </div>
    </div>
</body>

</html>